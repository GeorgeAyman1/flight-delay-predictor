...
columns = ['carrier_code','origin_airport','destination_airport','date_mmddyyyy','scheduled_departure_time','scheduled_elapsed_time_minutes','airline', 'year','flight_number']
...
invalid_time = (times.notna() & ((times < 0) | (times > 2359) | ((times % 100) >= 60))).sum()
print('Invalid scheduled departure time=',invalid_time)

invalid_origin = (~df["origin_airport"].astype(str).str.fullmatch(r"[A-Z]{3}") & df["origin_airport"].notna()).sum()
print("Invalid origin airport codes =", invalid_origin)

invalid_dest = (~df["destination_airport"].astype(str).str.fullmatch(r"[A-Z]{3}") & df["destination_airport"].notna()).sum()
print("Invalid destination airport codes =", invalid_dest)

invalid_carrier = (~df["carrier_code"].astype(str).str.fullmatch(r"[A-Z0-9]{2}") & df["carrier_code"].notna()).sum()
