...
invalidRelh = df[pd.to_numeric(df['relh'], errors='coerce').pipe(lambda s: (s < 0) | (s > 100))]
print('Humidity outside [0, 100] =', len(invalidRelh))

invalidDrct = df[pd.to_numeric(df['drct'], errors='coerce').pipe(lambda s: (s < 0) | (s > 360))]
...
invalidAlti = df[pd.to_numeric(df['alti'], errors='coerce').pipe(lambda s: (s < 25) | (s > 32))]
print('Altimeter outside [25, 32] inHg =', len(invalidAlti))

invalidMslp = df[pd.to_numeric(df['mslp'], errors='coerce').pipe(lambda s: (s < 850) | (s > 1100))]
print('Sea level pressure outside [850, 1100] mb =', len(invalidMslp))

invalidVsby = df[pd.to_numeric(df['vsby'], errors='coerce').pipe(lambda s: (s < 0) | (s > 100))]
